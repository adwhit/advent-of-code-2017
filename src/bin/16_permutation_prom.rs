...
named!(parse<Motion>, switch!(take!(1),
...
fn dance(motions: &[Motion]) -> String {
    use Motion::*;
    let mut seq: VecDeque<u8> = b"abcdefghijklmnop".iter().map(|v| *v).collect();
    for motion in motions {
        match *motion {
            Exchange(ix1, ix2) => seq.swap(ix1, ix2),
            Spin(amt) => {
                for _ in 0..amt {
                    let back = seq.pop_back().unwrap();
                    seq.push_front(back);
                }
            }
            Partner(n1, n2) => {
    ...
    String::from_utf8_lossy(&seq.into_iter().collect::<Vec<_>>()).into_owned()
}
...
fn get_data(path: &str) -> Result<Vec<Motion>> {
    ...
    s.split(',')
        .map(|v| {
            parse(v.trim().as_bytes()).to_result().map_err(|_| format_err!("parse fail: {:?}", v))
        })
        .collect()
}
